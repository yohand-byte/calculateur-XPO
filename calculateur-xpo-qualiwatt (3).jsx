...
import { Truck, DollarSign, TrendingDown, AlertCircle, Sun, Moon, Check, MapPin } from 'lucide-react';

export default function CalculateurXPO() {
  const tarifs = {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { icon: Truck, label: 'Tarif grille', value: calculs.tarifBase },
                { icon: DollarSign, label: 'Frais fixes', value: calculs.fraisFixesExpedition },
                { icon: TrendingDown, label: 'Montant gazole', value: calculs.montantGasoil },
              ].map((item, idx) => {
                const Icon = item.icon;
                return (
                  <div key={idx} className={`${colors.card} rounded-lg p-4`}>
                    <Icon className={`w-5 h-5 ${colors.accent} mb-2`} />
