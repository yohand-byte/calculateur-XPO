...
export default function XPOQualiwattProV3Final() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    ...
    const saved = localStorage.getItem('xpo-darkMode');
    return saved ? JSON.parse(saved) : false;
  });
  
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window === 'undefined') return 'tarif';
    return localStorage.getItem('xpo-activeTab') || 'tarif';
  });
  
  const [departement, setDepartement] = useState(() => {
    if (typeof window === 'undefined') return '35 - Ille et Vilaine';
    return localStorage.getItem('xpo-departement') || '35 - Ille et Vilaine';
  });
  
  const [quantite, setQuantite] = useState(() => {
    if (typeof window === 'undefined') return 0.5;
    const saved = localStorage.getItem('xpo-quantite');
    return saved ? parseFloat(saved) : 0.5;
  });
  
  const [premium, setPremium] = useState(() => {
    if (typeof window === 'undefined') return false;
    const saved = localStorage.getItem('xpo-premium');
    return saved ? JSON.parse(saved) : false;
  });
  
  const [target, setTarget] = useState(() => {
    if (typeof window === 'undefined') return false;
    const saved = localStorage.getItem('xpo-target');
    return saved ? JSON.parse(saved) : false;
  });
  
  const [priseRdv, setPriseRdv] = useState(() => {
    if (typeof window === 'undefined') return false;
    const saved = localStorage.getItem('xpo-priseRdv');
    return saved ? JSON.parse(saved) : false;
  });
  
  const [tauxGasoil, setTauxGasoil] = useState(() => {
    if (typeof window === 'undefined') return 0.0579;
    const saved = localStorage.getItem('xpo-tauxGasoil');
    return saved ? parseFloat(saved) : 0.0579;
  });

  const [adresses, setAdresses] = useState(() => {
    if (typeof window === 'undefined') return [addresseQualiwatt];
    const saved = localStorage.getItem('xpo-adresses');
    return saved ? JSON.parse(saved) : [addresseQualiwatt];
  });
  ...
  const [expeditions, setExpeditions] = useState(() => {
    ...
    const saved = localStorage.getItem('xpo-expeditions');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-darkMode', JSON.stringify(darkMode));
    }
  }, [darkMode]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-activeTab', activeTab);
    }
  }, [activeTab]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-departement', departement);
    }
  }, [departement]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-quantite', quantite.toString());
    }
  }, [quantite]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-premium', JSON.stringify(premium));
    }
  }, [premium]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-target', JSON.stringify(target));
    }
  }, [target]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-priseRdv', JSON.stringify(priseRdv));
    }
  }, [priseRdv]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-tauxGasoil', tauxGasoil.toString());
    }
  }, [tauxGasoil]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-adresses', JSON.stringify(adresses));
    }
  }, [adresses]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('xpo-expeditions', JSON.stringify(expeditions));
    }
  }, [expeditions]);
  ...
  // ===== CALCULS TARIF (GAZOLE = (GRILLE + FRAIS FIXES) * %) =====
  const calculs = useMemo(() => {
    const tarifBase = tarifs[departement]?.[quantite] || 0;
    const fraisFixesExpedition = 2.05;
    const sousTotal = tarifBase + fraisFixesExpedition; // (Grille + Frais fixes)
    const montantGasoil = sousTotal * tauxGasoil; // Gazole basé sur sous-total
    ...
    // Total HT = Grille + Frais fixes + Gazole + Premium + Target + RDV + IDF
    const totalHT = tarifBase + fraisFixesExpedition + montantGasoil + fraisPremium + fraisTarget + fraisPriseRdv + fraisIDF;
    return { tarifBase, fraisFixesExpedition, sousTotal, montantGasoil, fraisPremium, fraisTarget, fraisPriseRdv, estIDF, fraisIDF, totalHT };
    ...
    <div className={`min-h-screen ${colors.bg} transition-colors duration-300`}>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-xs ${colors.textSecondary}`}>v3.0 Final - Gestion Complète d'Expéditions</p>
            </div>
          </div>
          <button
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="flex gap-2 mb-8 border-b border-slate-200 dark:border-slate-800">
          {[
            { id: 'tarif', label: '📊 Calculateur Tarif' },
                ...
                <div className="space-y-3">
                  ...
                  {!newExpForm.showNewDestinationForm && (
                    <button
                      ...
                      className={`w-full py-2 border-2 border-dashed ${colors.accentBorder} rounded-lg text-sm font-semibold ${colors.accent} transition-all`}
                    >
                  ...
                  return (
                    <div key={exp.id} className={`${colors.card} rounded-xl p-5 space-y-3`}>
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleCopyExpJson(exp)}
                          className="flex-1 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded text-xs font-semibold hover:shadow-lg transition-all"
                        >
                          📤 Envoyer à XPO
                        </button>
                        {deletingExpId === exp.id ? (
                          <div className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-lg">
                            <div className={`${colors.card} rounded-lg p-4 space-y-3 border-2 border-red-500`}>
                              <p className={`text-sm font-bold ${colors.text}`}>Supprimer cette expédition ?</p>
                              <div className="flex gap-2">
                                <button
                                  onClick={() => handleDeleteExpedition(exp.id)}
                                  className="flex-1 py-2 bg-red-500 text-white rounded text-xs font-semibold hover:bg-red-600"
                                >
                                  Oui, supprimer
                                </button>
                                <button
                                  onClick={() => setDeletingExpId(null)}
                                  className={`flex-1 py-2 ${colors.card} rounded text-xs font-semibold`}
                                >
                                  Annuler
                                </button>
                              </div>
                          ...
                          <button
                            ...
                            className="flex-1 py-2 bg-red-500/10 hover:bg-red-500 text-red-600 hover:text-white rounded text-xs font-semibold transition-all border border-red-300 hover:border-red-500"
                          >
      ...
      <div className={`border-t ${colors.card} mt-12`}>
        <div className="max-w-7xl mx-auto px-6 py-8 text-center">
          <p className={`text-sm ${colors.textSecondary}`}>
            🚀 XPO Qualiwatt Pro v3.0 Final • Calcul exact • localStorage Sécurisé
          </p>
        </div>
      </div>
    </div>
